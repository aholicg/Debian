...
Không thể truy cập trực tiếp bởi phần mềm, mà được điều khiển ngầm bởi các lệnh chuyển đổi điều khiển (JMP, Jcc, CALL...), ngắt, và ngoại lệ.
...
main:
    ; Bước 1: Thực thi lệnh CALL. 
    ; Khi gọi hàm 'get_eip', CPU sẽ tự động đẩy địa chỉ của lệnh TIẾP THEO 
    ; (lệnh POP bên dưới) lên đỉnh Stack và nhảy vào hàm get_eip.
    call get_eip

next_instruction:
    ; Sau khi hàm trả về, thanh ghi EAX sẽ chứa giá trị của EIP tại thời điểm gọi.
    ; ... (tiếp tục xử lý giá trị trong EAX)
    ret

get_eip:
    ; Bước 2: Đọc địa chỉ trả về nằm trên đỉnh Stack.
    ; Lúc này, [ESP] chính là địa chỉ của nhãn 'next_instruction'.
    mov eax, [esp] 
    
    ; Bước 3: Trở về hàm gọi (lệnh RET sẽ lấy địa chỉ trên stack gán ngược lại vào EIP)
    ret
```
EIP có thể được nạp gián tiếp thông qua chỉnh sửa giá trị của con trỏ lệnh trả về trên đỉnh stack và thực thi lệnh trả về (RET/IRET).
